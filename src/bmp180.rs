...
where
    T: WriteRead + Read + Write + hal::prelude::_embedded_hal_async_i2c_I2c,
{
...
where
    T: I2c,
{
    pub async fn new(i2c: T /*, sleep_fn: fn(u32) -> ()*/) -> Bmp180<T> {
        println!("async"); // printed
        let mut i2c = i2c;
        let mut data = [0u8; 22];

        println!("async"); // printed
        i2c.write_read(ADDRESS, &[0xaa], &mut data).await.ok();

        println!("async"); // not printed
        let ac1 = ((data[0] as u16) << 8 | data[1] as u16) as i16 as i32;
        let ac2 = ((data[2] as u16) << 8 | data[3] as u16) as i16 as i32;
        let ac3 = ((data[4] as u16) << 8 | data[5] as u16) as i16 as i32;
        let ac4 = ((data[6] as u16) << 8 | data[7] as u16) as i16 as i32;
    ...
    pub async fn measure(&mut self) {
        // Select measurement control register
        // Enable temperature measurement
        self.i2c.write(ADDRESS, &[0xf4, 0x2e]).await.ok();
        println!("measure??");

        // Read 2 bytes of data from address 0xF6(246)
        // temp msb, temp lsb
        let mut data = [0u8; 2];
        self.i2c.write_read(ADDRESS, &[0xF6], &mut data).await.ok();
        ...
        // Select measurement control register
        // Enable pressure measurement, OSS = 1
        self.i2c.write(ADDRESS, &[0xf4, 0x74]).await.ok();
